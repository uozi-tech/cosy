...
Cosy 支持 INI、TOML、YAML 和 JSON 四种配置格式。以下是在不同格式中设置协议相关选项的示例：
...
### YAML 格式配置 (使用 yaml_settings 构建标签)

在 `app.yaml` 配置文件的 `server` 部分：
...
### JSON 格式配置 (使用 json_settings 构建标签)

在 `app.json` 配置文件的 `server` 部分：
